...
Route::get('event/reg-exp/{client}/{event}/{notes}', [ClientEventController::class, 'registerExpress'])->name('program.event.register-express')->withoutMiddleware(['auth', 'auth.department']);
Route::get('event/referral/{refcode}/{event_slug}', [ClientEventController::class, 'referralPage'])->name('program.event.referral-page')->withoutMiddleware(['auth', 'auth.department']);
