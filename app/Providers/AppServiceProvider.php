...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        view()->composer('*', function ($view) {
            ...
            if (isset($user) && (($user->departments()->wherePivot('status', 1)->count() > 0 || $user->roles()->where('role_name', 'Super Admin')->count() > 0))) {
                if (! Cache::has('menu')) {
                    foreach ($user->departments()->wherePivot('status', 1)->get() as $menus) {
    ...
    public function checkRoles($user, $collection)
    {
