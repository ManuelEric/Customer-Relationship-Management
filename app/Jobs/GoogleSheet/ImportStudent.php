    ...
    public function handle()
    {
        ...
        foreach ($this->studentData as $key => $val) {
            ...
            if (isset($val['Parents Name'])) {
                ...
                } elseif (! $checkExistParent['isExist']) {
        ...
        Sheets::spreadsheet(config('env.GOOGLE_SHEET_KEY_IMPORT'))->sheet(config('env.APP_ENV') == 'local' ? 'test student' : 'Students')->range('AA'.$this->studentData->first()['No'] + 1)->update($imported_date);
