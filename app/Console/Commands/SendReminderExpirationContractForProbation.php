...
class SendReminderExpirationContractForProbation extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:reminder_expiration_contracts_probation';
     ...
     * @var string
     */
    protected $description = 'Sending a reminder to HR Team if there are employees (probation) contract that must be renewed or almost finished.';
    ...
    {
        Log::info('Cron send reminder expiration contract probation working properly');
        ...
        try {
            Mail::send($mail_resources, ['list_contracts' => $list_contracts_expired_soon, 'title' => 'Probation'], function ($message) use ($subject) {
            ...
            Log::error('Failed to send expiration contract (probation) to HR Team caused by : ' . $e->getMessage() . ' | Line ' . $e->getLine());
