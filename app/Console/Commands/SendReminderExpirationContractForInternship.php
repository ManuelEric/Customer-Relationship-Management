...
class SendReminderExpirationContractForInternship extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'send:reminder_expiration_contracts_internship';
     ...
     * @var string
     */
    protected $description = 'Sending a reminder to HR Team if there are employees (internship) contract that must be renewed or almost finished.';
    ...
    {
        Log::info('Cron send reminder expiration contract internship working properly');
        ...
        try {
            Mail::send($mail_resources, ['list_contracts' => $list_contracts_expired_soon, 'title' => 'Internship'], function ($message) use ($subject) {
            ...
            Log::error('Failed to send expiration contract (internship) to HR Team caused by : ' . $e->getMessage() . ' | Line ' . $e->getLine());
