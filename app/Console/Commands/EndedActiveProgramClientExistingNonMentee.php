...
class EndedActiveProgramClientExistingNonMentee extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ended:client_program_existing_non_mentee';
     ...
     * @var string
     */
    protected $description = 'Ended all of the existing non mentee\'s client program when graduation year less than or equal to the current year';
    ...
    {
        $existing = $this->clientRepository->getExistingNonMentees();
        ...
        if (count($ids) == 0 ) {
            Log::notice('No client existing non-mentee found');
