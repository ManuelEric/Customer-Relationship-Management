        ...
        foreach ($program_buckets as $program_bucket) {
            ...
            switch ($param_name) {
                case 'School':
                    ...
                    $value_from_library = ProgramLeadLibrary::where('programbucket_id', $program_bucket_id)->
        ...
        foreach ($lead_buckets as $lead_bucket) {
            ...
            switch ($param_name) {
                case 'School':
                    ...
                    // find value from library
                    $value_from_library = ProgramLeadLibrary::where('leadbucket_id', $lead_bucket_id)->
                                                where('value_category', $value_of_field)->
                                                pluck($type)->first();
            ...
            if ($program_score <= 0.34) {
                $total_result = 0;
            } elseif ($program_score >= 0.35 && $client->lead_source == 'Referral') {
                $total_result = 1;
            }
