...
class DeleteSchoolProgramAttachAction
{
    ...
    )
    {
        ...
        if (Storage::disk('s3')->exists('project/crm/attachment/school_prog_attach/'. $school_prog_attach->schprog_id . '/' . $school_prog_attach->schprog_attach)) {

            if ($this->schoolProgramAttachRepository->deleteSchoolProgramAttach($attach_id)) {
                Storage::disk('s3')->delete('project/crm/attachment/school_prog_attach/'. $school_prog_attach->schprog_id . '/' . $school_prog_attach->schprog_attach);
            }
        } else {
