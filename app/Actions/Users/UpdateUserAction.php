...
class UpdateUserAction
{
    ...
    )
    {
        ...
        $CV_file_path = $this->tnUploadFile($request, 'curriculum_vitae', 'CV-' . str_replace(' ', '_', $request->first_name . '_' . $request->last_name), 'project/crm/user/' . $user_id . '/');


        # 7. upload KTP / idcard
        $ID_file_path = $this->tnUploadFile($request, 'idcard', 'ID-' . str_replace(' ', '_', $request->first_name . '_' . $request->last_name), 'project/crm/user/' . $user_id . '/');


        # 8. upload tax
        $TX_file_path = $this->tnUploadFile($request, 'tax', 'TAX-' . str_replace(' ', '_', $request->first_name . '_' . $request->last_name), 'project/crm/user/' . $user_id . '/');


        # 9. upload bpjs kesehatan / health insurance
        $HI_file_path = $this->tnUploadFile($request, 'health_insurance', 'HI-' . str_replace(' ', '_', $request->first_name . '_' . $request->last_name), 'project/crm/user/' . $user_id . '/');


        # 10. upload bpjs ketenagakerjaan / empl insurance
        $EI_file_path = $this->tnUploadFile($request, 'empl_insurance', 'EI-' . str_replace(' ', '_', $request->first_name . '_' . $request->last_name), 'project/crm/user/' . $user_id . '/');
