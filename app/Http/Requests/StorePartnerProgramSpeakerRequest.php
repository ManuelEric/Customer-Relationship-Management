...
class StorePartnerProgramSpeakerRequest extends FormRequest
{
    /**
    ...
    protected function store()
    {
        ...
        $rules = [
            ...
            'start_time' => [
                'required',
                // function ($attribute, $value, $fail) use ($partnerProgId, $endTime) {
                //     if (!Event::where('event_startdate', 'like', $value.'%')->whereEventId($partnerProgId)) {

                //         $fail('The start time is invalid');

                //     } elseif (AgendaSpeaker::where('event_id', $partnerProgId)->whereBetween('start_time', [date('Y-m-d H:i:s', strtotime($value)), date('Y-m-d H:i:s', strtotime($endTime))])->count() > 0) {
            ...
            'end_time' => [
                'required',
                // function ($attribute, $value, $fail) use ($partnerProgId, $startTime) {
                //     if (!Event::where('event_enddate', 'like', $value.'%')->whereEventId($partnerProgId)) {

                //         $fail('The start time is invalid');

                //     } elseif (AgendaSpeaker::where('event_id', $partnerProgId)->whereBetween('end_time', [date('Y-m-d H:i:s', strtotime($startTime)), date('Y-m-d H:i:s', strtotime($value))])->count() > 0) {
        ...
        switch ($this->input('speaker_type')) {
            case "internal":
                $rules['allin_speaker'][] = function ($attribute, $value, $fail) use ($partnerProgId) {
                    ...
                    } elseif ($this->input('speaker_type') == 'internal' && AgendaSpeaker::where('empl_id', $value)->where('sch_prog_id', $partnerProgId)->where('start_time', '=', $this->input('start_time'))->where('end_time', '=', $this->input('end_time'))->first()) {
