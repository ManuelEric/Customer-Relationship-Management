...
<script>
    function showAlert(data)
    {
        ...
        const achieved_percentage_of_participant = target_of_participant == 0 ? actual_of_participant * 100 : (actual_of_participant / target_of_participant) * 100;
        ...
        const achieved_percentage_of_amount = target_of_amount == 0 ? actual_of_amount * 100 : (actual_of_amount / target_of_amount) * 100;
        ...
        if (target_of_participant < actual_of_participant && target_of_amount < actual_of_amount) {
            ...
            var symbol = '<svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Success:"><use xlink:href="#check-circle-fill"/></svg>';
            var text = 'Congratulations! You\'ve achieved the target by ' + achieved_percentage_of_participant + '% on client and ' + achieved_percentage_of_amount + '% on amount';
        } else if (target_of_participant < actual_of_participant || target_of_amount < actual_of_amount) {
            var alert_type = 'alert-warning';
            var symbol = '<svg class="bi flex-shrink-0 me-2" width="24" height="24" role="img" aria-label="Warning:"><use xlink:href="#exclamation-triangle-fill"/></svg>';
            var text = '<p>You\'ve achieved the target but please remember that the other target is not achieved yet. Good Luck!</p>';
        } else {
        ...
        var html = '<div class="alert '+ alert_type +' d-flex align-items-center" role="alert">' +
                    '<div class="d-flex flex-column">' +
                    '<div class="d-flex w-full">'+ symbol + text +'</div>' +
                    '<hr><p class="mb-0"> Target of client : <b>'+ round_percentage_participant +'%</b> & target of amount : <b>'+ round_percentage_amount +'%</b></p></div>' +
                '</div>';
