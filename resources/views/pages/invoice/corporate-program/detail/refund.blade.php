<div class="card mb-3">
    ...
    <div class="card-body">
        @if (isset($invoicePartner->refund))
            <b style="font-size: 1.5em">Total refund : {{ $invoicePartner->refund->total_refunded_str }}</b>
            <hr>
            <p style="font-size: 1em">
                with detail: <br>
                <ul style="font-size: 1em">
                    <li>Total Paid : {{ $invoicePartner->refund->total_paid_str }}</li>
                    <li>Refund Amount : {{ $invoicePartner->refund->refund_amount_str.' ('.$invoicePartner->refund->percentage_refund.'%)' }}</li>
                    <li>Tax : {{ $invoicePartner->refund->tax_amount_str.' ('.$invoicePartner->refund->tax_percentage.'%)' }}</li>
                </ul>
            </p>
        @else
            <p>
                Reason : {{ $invoicePartner->partner_prog->reason->reason_name }}
            </p>
            {!! isset($invoicePartner->partner_prog->refund_notes) ? $invoicePartner->partner_prog->refund_notes : 'If the client wasn\'t going to continue the program. You can click the button below'!!}
        @endif
        <br>
    ...
    {{-- @if(isset($invoicePartner)) --}}
        {{-- @if($invoicePartner->invb2b_status == 2)
            <div class="card-footer d-flex justify-content-between">
                <h6 class="m-0 p-0">Total Refund</h6>
                <h6 class="m-0 p-0">{{ $invoicePartner->refund->total_refunded_str }}</h6>
            </div> 
        @endif --}}
    {{-- @endif --}}
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="{{ isset($invoicePartner) ? route('invoice-corp.refund', ['invoice' => $invoicePartner->invb2b_num]) : '' }}" method="POST">
                    @csrf
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label for="">Total Price </label>
                            <input type="number" name="total_payment" id="" 
                            value="{{ $invoicePartner->invb2b_totpriceidr }}" readonly
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="total_paid" id="total_paid"
                                value="{{ $invoicePartner->receipt()->sum('receipt_amount_idr') }}"
                                readonly
                                class="form-control form-control-sm rounded">
                                @error('total_paid')
                                    <small class="text-danger fw-light">{{ $message }}</small>
...
<div class="modal fade" id="cancel_refund">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ isset($invoicePartner->refund) ? route('invoice-corp.refund.destroy', ['invoice' => $invoicePartner->invb2b_num, 'refund' => $invoicePartner->refund->id]) : '' }}" method="POST">
                    @method('delete')
                    @csrf
