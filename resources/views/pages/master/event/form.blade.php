    ...
    <div class="row">
            ...
            <div class="card rounded mb-3">
                <div class="card-body">
                    ...
                    <div class="mt-3 d-flex justify-content-center">
                        <a href="{{ route('event.edit', ['event' => $event->event_id]) }}" class="btn btn-sm btn-outline-info rounded mx-1">
                            <i class="bi bi-pencil"></i> Edit
                        </a>
                        <button type="button" class="btn btn-sm btn-outline-danger rounded mx-1" onclick="confirmDelete('master/event', '{{ $event->event_id }}')">
        ...
        <div class="col-md-8">
            <div class="card rounded mb-3">
                ...
                <div class="card-body">
                    <form action="{{ isset($event) ? route('event.update', ['event' => $event->event_id]) : route('event.store') }}" method="POST">
                        @csrf
                        @if (isset($event)) 
                            @method('PUT')
                        @endif
                        <div class="row">
                            <div class="col-md-12 mb-2">
                                ...
                                <input type="text" name="event_title" value="{{ isset($event->event_title) ? $event->event_title : null }}"
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="datetime-local" name="event_startdate" value="{{ isset($event->event_startdate) ? $event->event_startdate : null }}"
                                    class="form-control form-control-sm rounded" {{ $disabled }}>
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="datetime-local" name="event_enddate" value="{{ isset($event->event_enddate) ? $event->event_enddate : null }}"
                                    class="form-control form-control-sm rounded" {{ $disabled }}>
                                @error('event_enddate')
                                    <small class="text-danger fw-light">{{ $message }}</small>
                                @enderror
                            ...
                            <div class="col-md-12 mb-2">
                                ...
                                <select name="user_id[]" multiple class="select w-100" {{ $disabled }}>
                                    ...
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ isset($eventPic) ? (in_array($employee->id, $eventPic) ? "selected" : null) : null }}>{{ $employee->first_name.' '.$employee->last_name }}</option>
                                    @endforeach
                                </select>
                                @error('event_enddate')
                                    <small class="text-danger fw-light">{{ $message }}</small>
