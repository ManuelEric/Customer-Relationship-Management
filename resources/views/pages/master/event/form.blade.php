                    ...
                    <form action="{{ isset($event) ? route('event.update', ['event' => $event->event_id]) : route('event.store') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-2">
                                ...
                                <input type="text" name="event_title" value="{{ isset($event->event_title) ? $event->event_title : old('event_title') }}"
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="datetime-local" name="event_startdate" value="{{ isset($event->event_startdate) ? $event->event_startdate : old('event_startdate') }}"
                                    class="form-control form-control-sm rounded" {{ $disabled }}>
                            ...
                            <div class="col-md-6 mb-2">
                                ...
                                <input type="datetime-local" name="event_enddate" value="{{ isset($event->event_enddate) ? $event->event_enddate : old('event_enddate') }}"
                                    class="form-control form-control-sm rounded" {{ $disabled }}>
                                @error('event_enddate')
                                    <small class="text-danger fw-light">{{ $message }}</small>
                                @enderror
                                ...
                                <select name="user_id[]" multiple class="select w-100" {{ $disabled }}>
                                    ...
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" 
                                            @if (isset($eventPic))
                                                {{ in_array($employee->id, $eventPic) ? "selected" : null }}
                                            @elseif (old('user_id'))
                                                {{ in_array($employee->id, old('user_id')) ? "selected" : null }}
                                            @endif
