<div class="card {{ $is_tutor || $is_external_mentor || $is_editor ? null : 'd-none' }}" id="subject">
        ...
        <div class="row" id="subjectField">
            {{-- <div class="col-md-12 education"> --}}
                @if (isset($user) && count($user->user_subjects) > 0 )
                        ...
                        <div class="row border py-2 mx-1 mb-1 subject subject-{{ $loop->index }}">
                            <div class="col-md-12">
                                <div class="row g-2">
                                    ...
                                    @if($is_external_mentor || $is_editor)
                                        <div class="col-md-4 mb-3">
                                            <label for="" class="text-muted">Type <sup class="text-danger">*</sup></label>
                                            <select name="role_type[]" class="select w-100">
                                                <option data-placeholder="true"></option>
                                                @if($is_editor || Request::route('user_role') == 'editor')
                                                    <option value="Essay Mentor" @selected('Essay Mentor' == $user_subject->subject->type)>Essay Mentor</option>
                                                    <option value="Essay Editor" @selected('Essay Editor' == $user_subject->subject->type)>Essay Editor</option>
                                                @else
                                                    @foreach ($role_type_mentors as $role_type_mentor)
                                                        <option value="{{ $role_type_mentor }}" @selected($role_type_mentor == $user_subject->subject->type)>{{ $role_type_mentor }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @error('role_type.'.$loop->index)
                                                <small class="text-danger fw-light">{{ $message }}</small>
                                            @enderror
                                        </div>
                                    @elseif($is_tutor)
                    ...
                    @endforeach
                @elseif (old('count_agreement') !== null)
                    @for($i = 0 ; $i < count(old('count_agreement')) ; $i++)
                        <div class="row border py-2 mx-1 mb-1 subject">
                            <div class="col-md-12">
                                <input type="hidden" value="1" name="count_agreement[]">
                                <div class="row g-2">
                                    <div class="col-md-4 mb-3">
                                        <label for="" class="text-muted">Agreement <sup class="text-danger">*</sup></label>
                                    ...
                                    @if($is_external_mentor || $is_editor)
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <select name="role_type[]" class="select w-100">
                                                <option data-placeholder="true"></option>
                                                @if($is_editor || Request::route('user_role') == 'editor')
                                                    <option value="Essay Mentor" @selected('Essay Mentor' == old('type.'.$i))>Essay Mentor</option>
                                                    <option value="Essay Editor" @selected('Essay Editor' == old('type.'.$i))>Essay Editor</option>
                                                @else
                                                    @foreach ($role_type_mentors as $role_type_mentor)
                                                        <option value="{{ $role_type_mentor }}" @selected($role_type_mentor == old('type.'.$i))>{{ $role_type_mentor }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @error('role_type.'.$i)
                                    ...
                                    <div class="detail-subject border py-2 px-3 mb-1" id="detail-subject-{{ $i }}">
                                        @for ($j=0; $j < count(old('count_agreement_detail.'.$i)); $j++)
                                                <div class="row" id="subjectDetailField-{{$j}}">
                                                    <input type="hidden" value="1" name="count_agreement_detail[{{$i}}][]">
                                                    @if($is_tutor || Request::route('user_role') == 'tutor')
                                                        <div class="col-md-2 mb-3">
                                                            <label for="" class="text-muted">Grade <sup class="text-danger">*</sup></label>
                        ...
                        <div class="row g-2">
                            ...
                            @if($is_external_mentor || $is_editor)
                                <div class="col-md-4 mb-3">
                                    ...
                                    <select name="role_type[]" class="select w-100">
                                        <option data-placeholder="true"></option>
                                        @if($is_editor)
                                            <option value="Essay Mentor">Essay Mentor</option>
                                            <option value="Essay Editor">Essay Editor</option>
                                        @else
                                            @foreach ($role_type_mentors as $role_type_mentor)
                                                <option value="{{ $role_type_mentor }}">{{ $role_type_mentor }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                    @error('role_type.0')
                            ...
                            <div class="detail-subject border py-2 px-3 mb-1" id="detail-subject-0">
                                <div class="row" id="subjectDetailField-0">
                                    <input type="hidden" value="1" name="count_agreement_detail[0][]">
                                        @if($is_tutor)
                                            <div class="col-md-2 mb-3">
                                                <label for="" class="text-muted">Grade <sup class="text-danger">*</sup></label>
            ...
            '<div class="row border py-2 mx-1 mb-1 subject subject-' + id + '">' +
                ...
                '<div class="row g-2">' +
                    ...
                    @if($is_external_mentor || $is_editor)
                        '<div class="col-md-4 mb-3">' +
                            '<label for="" class="text-muted">Type <sup class="text-danger">*</sup></label>' +
                            '<select name="role_type[]" id="" class="select w-100">' +
                            '<option data-placeholder="true"></option>' +
                            @if($is_editor)
                                '<option value="Essay Mentor" selected>Essay Mentor</option>' +
                                '<option value="Essay Editor">Essay Editor</option>' +
                            @else
                                @foreach ($role_type_mentors as $role_type_mentor)
                                    '<option value="{{ $role_type_mentor }}" selected>{{ $role_type_mentor }}</option>' +
                                @endforeach
                            @endif
                            @error('role_type.0')
                                '<small class="text-danger fw-light">{{ $message }}</small>' +
                    ...
                    '<div class="detail-subject border py-2 mx-1 mb-1" id="detail-subject-'+index+'">' +
                        '<div class="row" id="subjectDetailField-'+index+'">' +
                            '<input type="hidden" value="1" name="count_agreement_detail['+index+'][]">' +
                                @if ($is_tutor)
                                    '<div class="col-md-2 mb-3">' +
                                        '<label for="" class="text-muted">Grade <sup class="text-danger">*</sup></label>' +
