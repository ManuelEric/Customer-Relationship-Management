...
@endphp
    ...
    <div class="row">
        <div class="col-md-3 mb-3">
            <div class="card rounded mb-3">
                <div class="card-body">
                    ...
                    <div class="text-center mt-2">
                        @if (isset($user))
                            <button @class(['btn btn-sm btn-success', 'd-none' => $user->active == 1]) id="activate-user" style="font-size:12px;">
                            ...
                            <button id="set-password" @class(['btn btn-sm btn-warning', 'd-none' => $user->active == 0]) style="font-size:12px;">
                                <i class="bi bi-key"></i>
                                Set Password
                            </button>
                        @endif
                                ...
                                <div @class([
                                    ...
                                    <div>
                                        @if ($type->pivot->status == 1)
                                            <button
                                                onclick="confirmDelete('{{ 'user/' . Request::route('user_role') . '/' . Request::route('user') }}', '{{ $type->pivot->id }}')"
            ...
            @if (isset($user) &&
                    (in_array(4, $user->roles()->pluck('role_id')->toArray()) ||
                        in_array(3, $user->roles()->pluck('role_id')->toArray()) ||
                        in_array(19, $user->roles()->pluck('role_id')->toArray()) ||
                        in_array(20, $user->roles()->pluck('role_id')->toArray()) ||
                        in_array(13, $user->roles()->pluck('role_id')->toArray()) ||
                        in_array(14, $user->roles()->pluck('role_id')->toArray()) ||
                        in_array(15, $user->roles()->pluck('role_id')->toArray())))
        ...
        <div class="col-md-9">
            <div class="card rounded mb-3">
                ...
                <div class="card-body">
                    <form
                        action="{{ isset($user) ? route('user.update', ['user_role' => Request::route('user_role'), 'user' => $user->id]) : route('user.store', ['user_role' => Request::route('user_role')]) }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="date" name="datebirth"
                                    value="{{ isset($user->datebirth) ? $user->datebirth : old('datebirth') }}"
                            ...
                            <div class="col-md-12 mb-3">
                                <div class="card">
                                    <div class="card-header py-2 d-flex justify-content-between align-items-center">
                                        <h6 class="m-0 p-0"><i class="bi bi-exclamation-diamond"></i> &nbsp; Emergency
                                            Contact Information</h6>
                                    </div>
                                    <div class="card-body">
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                <label for="">Phone Number {!! Request::route('user_role') == 'employee' || Request::route('user_role') == 'admin'
                                                    ? '<sup class="text-danger">*</sup>'
                                                    : '' !!}</label>
                                                <input type="text" name="emergency_contact_phone"
                                                    value="{{ isset($user->emergency_contact_phone) ? $user->emergency_contact_phone : old('emergency_contact_phone') }}"
                                                    class="form-control form-control-sm rounded"
                                            ...
                                            <div class="col-md-6 mb-3">
                                                <label for="">Relation {!! Request::route('user_role') == 'employee' || Request::route('user_role') == 'admin'
                                                    ? '<sup class="text-danger">*</sup>'
                                                    : '' !!}</label>
                                                <input type="text" name="emergency_contact_relation_name"
                                                    value="{{ isset($user->emergency_contact_relation_name) ? $user->emergency_contact_relation_name : old('emergency_contact_relation_name') }}"
                                                    class="form-control form-control-sm rounded"
                                                    placeholder="Brother / Mother / Father / etc">
                                                @error('emergency_contact_phone')
                                                    <small class="text-danger fw-light">{{ $message }}</small>
                                                @enderror
                                            </div>
    ...
    <script type="text/javascript">
        ...
        @if (isset($user))
            ...
            function deactivateUser(status) {
                ...
                axios.post(
                        '{{ route('user.update.status', ['user_role' => Request::route('user_role'), 'user' => $user->id]) }}', {
                            _token: '{{ csrf_token() }}',
                            new_status: status,
        ...
        @if (isset($user))
            ...
            $(".curriculum-vitae-container .download").on('click', function() {
                window.open(
                    "{{ route('user.file.download', ['user_role' => Request::route('user_role'), 'user' => $user->id, 'filetype' => 'CV']) }}",
                    '_blank');
            })
            ...
            $(".ktp-container .download").on('click', function() {
                window.open(
                    "{{ route('user.file.download', ['user_role' => Request::route('user_role'), 'user' => $user->id, 'filetype' => 'ID']) }}",
                    '_blank');
            })
            ...
            $(".tax-container .download").on('click', function() {
                window.open(
                    "{{ route('user.file.download', ['user_role' => Request::route('user_role'), 'user' => $user->id, 'filetype' => 'TX']) }}",
                    '_blank');
            })
            ...
            $(".hi-container .download").on('click', function() {
                window.open(
                    "{{ route('user.file.download', ['user_role' => Request::route('user_role'), 'user' => $user->id, 'filetype' => 'HI']) }}",
                    '_blank');
            })
            ...
            $(".ei-container .download").on('click', function() {
                window.open(
                    "{{ route('user.file.download', ['user_role' => Request::route('user_role'), 'user' => $user->id, 'filetype' => 'EI']) }}",
                    '_blank');
            })
